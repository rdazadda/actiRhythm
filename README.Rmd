...
cross-checks the metrics live against nparACT, ActCR and the cosinor package; and the
method-family articles (nonparametric, cosinor, period, fractal, sleep, phase, raw
acceleration, batch) each work one family from the math to a worked example. Every
function's help page also carries its method reference, so `?circadian.rhythm`,
`?cosinor.analysis`, and `?circadian.period` are good places to read more. Questions
and bug reports go to the issue tracker at

...
equals the standard error of each coefficient
\\\sqrt{\mathrm{Var}(\beta)}\\, which is how \\\Sigma\\ is reconstructed
when it is not supplied explicitly. The point estimate is recovered from
amplitude and acrophase as \\\beta_1 = A cos(\phi)\\, \\\beta_2 = A
sin(\phi)\\ with \\\phi = acrophase \cdot 2\pi / T\\.
